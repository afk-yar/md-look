...
подставляется содержимое Markdown-файла, имя и папка документа. Шаблон несёт
собственный Markdown-парсер, KaTeX, Mermaid и полноразмерный просмотр
(lightbox) картинок и диаграмм.

Lightbox — оверлей `#lightbox` в шаблоне. Собирает ленту из `img` и
отрендеренных `.mermaid-wrap svg` текущего контейнера (`#readerContent` в Read,
`#preview` в Edit) в порядке DOM, показывает элемент с зумом и панорамой через
CSS `transform`. Диаграмма попадает в оверлей клоном SVG, картинка — тем же
`src`. Клавиши перехватываются в capture-фазе, чтобы `Esc` и стрелки не
доставались поиску, Zen-режиму и телесуфлёру.
...
Single-instance — наличие уже запущенного экземпляра определяется именованным
Win32 mutex `Local\MDLook-<derived-port>` (`_acquire_instance_mutex`). Номер в
имени и порт детерминированно выводятся из пути исполняемого файла или
dev-скрипта, чтобы установленная копия и portable/dev-сборка не перехватывали
запуски друг друга. Первый экземпляр держит handle mutex до конца жизни
процесса; ОС освобождает mutex при завершении, поэтому зависший процесс не
блокирует следующий запуск.

TCP-канал (`127.0.0.1:<derived-port>`) используется только для передачи команд:
второй запуск той же копии, увидев занятый mutex, шлёт первому процессу `SHOW`
или `OPEN:<path>[\t<goto-target>]`. Если mutex свободен, сокет вообще не
трогается — раньше стартовая проба порта висела до таймаута (~1 с на этой
машине, connect к закрытому localhost-порту не получает RST). Гонка «mutex уже
создан, listener ещё не забиндил порт» покрыта короткими повторами connect
(0.5 с на попытку, до ~3 с суммарно).
